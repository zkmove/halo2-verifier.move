...
#[test_only]
module halo2_verifier::gwc_test {
    use sui::bn254;
    ...
    fun scalar(value: u64): sui::group_ops::Element<bn254::Scalar> {
    ...
    fun transcript_with_witness(): transcript::Transcript {
        let witness = bn254::g1_mul(&scalar(3), &bn254::g1_generator());
        transcript::new(serialize_g1(&witness))
    }
    ...
    fun test_verify_single_opening() {
        ...
        let mut transcript = transcript_with_witness();

        assert!(gwc::verify(&params, &mut transcript, &queries), 1000);
    ...
    fun test_verify_rejects_wrong_eval() {
        ...
        assert!(!gwc::verify(&params, &mut transcript, &queries), 2000);
    }
}
