...
#[test_only]
module halo2_verifier::shuffle_test {
    use sui::bn254;
    use sui::group_ops;
    ...
    fun proof_for_shuffle(product_commitment: &group_ops::Element<bn254::G1>): vector<u8> {
        let mut proof = serialize_g1(product_commitment);
    ...
    fun test_read_evaluate_and_queries() {
        ...
        let mut transcript = transcript::new(proof_for_shuffle(&product_commitment));

        let commited = shuffle::shuffles_read_product_commitments(&mut transcript);
        let evaluated = shuffle::evaluate(&commited, &mut transcript);
        ...
        shuffle::queries(&vector[evaluated], &mut queries, &protocol, &d, &x);
        ...
        assert!(group_ops::equal(query::eval(&queries[1]), &scalar(7)));
    }

    #[test]
    fun test_expression_single_shuffle() {
        let g = bn254::g1_generator();
        let product_commitment = bn254::g1_mul(&scalar(3), &g);
        let mut transcript = transcript::new(proof_for_shuffle(&product_commitment));
        ...
        let evaluated = shuffle::evaluate(&commited, &mut transcript);
        let protocol = sample_protocol();
        let shuffle_meta = &protocol::shuffles(&protocol)[0];
        ...
        assert!(group_ops::equal(&bn254::scalar_add(&results[2], &scalar(342)), &bn254::scalar_zero()), 2003);
    }

    #[test, expected_failure]
    fun test_read_product_commitment_short_proof_aborts() {
        let mut transcript = transcript::new(vector[1, 2, 3]);
        let _ = shuffle::shuffles_read_product_commitments(&mut transcript);
    }
}
