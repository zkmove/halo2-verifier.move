...
#[test]
fun test_digest_mismatch_returns_false_through_bytes_helper() {
    let empty = x"";
    let malformed = x"01";
    ...
    assert!(!native_verifier::verify_proof_bytes(
        malformed,
        wrong_digest,
        copy empty,
        copy empty_digest,
...
fun test_oversize_public_inputs_abort_in_api() {
    ...
    native_verifier::verify_proof_bytes(
        copy empty,
        copy empty_digest,
        copy empty,
        copy empty_digest,
...
fun test_oversize_params_abort_on_object_creation() {
    ...
    let params = serialized_params_store::new_serialized_params(
        zero_bytes(serialized_params_store::max_params_bytes() + 1),
        ctx,
    );
    serialized_params_store::destroy(params);
}
