...
module halo2_verifier::transcript_blake2b {
    ...
    public fun init(input: vector<u8>): Transcript {
        ...
        let state = blake2b::new();
        blake2b::update(&mut state, b"Halo2-Transcript");
        Transcript {
            state,
            reader: Read {
                buf: input,
    ...
    fun common_scalar_serialized(self: &mut Transcript, bytes: vector<u8>) {
        blake2b::update(&mut self.state, vector::singleton(BLAKE2B256_PREFIX_SCALAR));
        blake2b::update(&mut self.state,bytes);
    }
    ...
    public fun common_point(self: &mut Transcript, point: Element<G1>) {
        blake2b::update(&mut self.state, vector::singleton(BLAKE2B256_PREFIX_POINT));
        ...
        blake2b::update(&mut self.state, le_repr);
    }
    ...
    public fun squeeze_challenge(self:  &mut Transcript) : Element<Fr> {
        blake2b::update(&mut self.state, vector::singleton(BLAKE2B256_PREFIX_CHALLENGE)); 

        let state_lo : Blake2b = self.state;
        let state_hi : Blake2b = self.state;
        blake2b::update(&mut state_lo, vector::singleton(BLAKE2B256_PREFIX_CHALLENGE_LO));
        blake2b::update(&mut state_hi, vector::singleton(BLAKE2B256_PREFIX_CHALLENGE_HI));

        let result_lo = blake2b::finalize(&mut state_lo);
        let result_hi = blake2b::finalize(&mut state_hi);
